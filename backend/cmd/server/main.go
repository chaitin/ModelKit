...
import (
	...
	"github.com/chaitin/ModelKit/backend/docs"
	"github.com/chaitin/ModelKit/backend/internal"
	"github.com/chaitin/ModelKit/backend/pkg"
	"github.com/chaitin/ModelKit/backend/pkg/service"
	"github.com/chaitin/ModelKit/backend/pkg/store"
)

// @title ModelKit API
// @version 1.0
// @description ModelKit API
func main() {
	s, err := newServer()
	if err != nil {
		panic(err)
	...
	if s.config.Debug {
		s.web.Swagger("ModelKit API", "/reference", string(docs.SwaggerJSON), web.WithBasicAuth("mc", "mc88"))
	}
