...
func (p *Panda) GetModelList(c echo.Context) error {
	...
	resp, err := usecase.PandaModelList(c.Request().Context(), &req)
	if err != nil {
...
func (p *Panda) CheckModel(c echo.Context) error {
	...
	resp, err := usecase.PandaCheckModel(c.Request().Context(), &req)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, domain.Response{
			Success: false,
			Message: err.Error(),
